...
// Fixed ione-lavender palette — pulled from landing/src/components/InteractiveGradient.tsx
// so the orb reads as the same flower across the marketing site and the tutor workspace.
//
//   #ece7f5  cream-lavender base   (matches body bg / fades into UI)
//   #ddd6fe  violet-200 highlight  (soft halo near gradient peaks)
//   #a78bfa  violet-400 / lavender (mid tone, ione petal)
//   #7c3aed  violet-600 deep       (core particles, accent)
//   #6366f1  indigo-500            (cool counterpoint, prevents pink drift)
...
void main() {
  ...
  gl_PointSize = (1. - (pos.w / 100.)) * 3.0 * dpr * position.z;
}
...
// Fewer particles than 512² — same look, much less overdraw in the center.
const width = 384;
const height = 384;
...
function interpolate(time, renderer) {
  ...
  // Interpolate the bkg color uniform (still drives shader math), but keep
  // the actual canvas alpha at 0 so the orb floats over whatever React paints
  // behind the iframe (the cream-lavender card surface in TutorWorkspace).
  bkg.copy(bkgFrom).lerp(bkgTo, t);
  renderer.setClearColor(bkg, 0);
}
