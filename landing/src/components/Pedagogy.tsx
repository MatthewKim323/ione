      ...
      <div className="relative z-10 max-w-[1380px] mx-auto">
        ...
        <div className="mt-12 grid grid-cols-1 lg:grid-cols-12 gap-x-12 gap-y-16">
          ...
          <div className="lg:col-span-6 lg:pt-4 space-y-7">
            <motion.p
              ...
              className="text-ink/80 text-[15px] leading-[1.7] font-sub max-w-[52ch]"
            >
              The best math tutors are mostly silent. They sit beside the
              student, watch the work{" "}
              <strong className="text-ink font-bold">unfold</strong>, and
            ...
            >
              ...
              <strong className="text-ink font-bold">asymmetry</strong>.
        ...
        <div className="mt-24 grid grid-cols-2 sm:grid-cols-4 gap-y-10 border-t border-ink-line pt-10">
          {STATS.map((s, i) => (
            ...
            >
              <span
                className="text-ink text-[clamp(2.4rem,4vw,3.6rem)] leading-none"
                style={{ fontFamily: "var(--font-display)" }}
              >
                {s.n}
              </span>
              <span className="meta-label text-ink/50">{s.label}</span>
            </motion.div>
          ))}
        </div>

        {/* pillars — 3D tilt (pointer-driven, spring) */}
        <div className="mt-28 grid grid-cols-1 md:grid-cols-2 gap-6 md:gap-7">
          {PILLARS.map((p, i) => (
            <PedagogyPillarCard key={p.title} index={i}>
              <div className="flex items-baseline justify-between">
                <span
                  className="text-ink text-[1.6rem] leading-none"
